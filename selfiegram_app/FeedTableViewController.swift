...
//  selfiegram_app
//
//  Created by Alvin Phun on 2018-10-09.
...
class FeedTableViewController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func doubleTappedSelfie(_ sender: UITapGestureRecognizer) {
        ...
        // based on x,y position we can get the indexPath for where we are at
        if let indexPathAtLocation = tableView.indexPathForRow(at: tapLocation) {
            // based on the indexPath we can get the specific cell that is being tapped
            let cell = tableView.cellForRow(at: indexPathAtLocation) as! SelfieCell
            ...
            if let imageData = UIImageJPEGRepresentation(image, 0.9),
                ...
                post.saveInBackground(block: {(success, error) -> Void in
                    if success {
                        ...
                        // 3. Add post to our posts array, choose index 0 so that it will be added to the top of your table instead of at the bottom (default behavior)
                        self.posts.insert(post, at: 0)
                        
                        // 4. Now that we have added a post, updating our table. We are just inserting our new Post instead of reloading our whole tableView. Both method would work, however this gives us a cool animation for free
